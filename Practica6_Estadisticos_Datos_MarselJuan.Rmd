...
ruta<-paste0("data/",archivos[1])
Datos<- read_tsv(ruta,col_names = TRUE)

Datos<-Datos %>% 
  mutate(across(
    ends_with(".cat"),as.factor
  ))
...
Empleando la función `sumarise(across())` calcula la media, la desviación típica, mediana y el *interquartile range* (`IQR`) de las variables que acaban en `.num` (notas numéricas).
...
```{r}
Estadisticos<-Estadisticos %>% pivot_longer(
    cols = everything(),         # toma todas las columnas
    names_to = "asignatura", # columna con los nombres combinados
  ...
  mutate(
    across(
      c(media, desvest, mediana, IQR),
      ~ round(., digits=2)
      )
    ) %>%
  
  # combinar mean y sd en una columna de texto mean±sd
  unite(msd, media, desvest, sep = "±") %>%
  
  # ordenar columnas como pide el enunciado
  select(asignatura, msd, mediana, IQR)
...
ggplot(Datos, aes(x=ALG.num)) + 
  ...
  geom_vline(aes(xintercept=mean(ALG.num))) + 
  geom_density() +
  theme_minimal()
```
...
Tabla1<-table(Datos$ALG.cat)

Tabla2 <- table(Datos$ALG.cat, Datos$ANM.cat)
Tabla2

mosaicplot(
  Tabla2,
  main = "Relación entre ALG y ANM",
  xlab = "ALG",
  ylab = "ANM"
)
...
# Nos quedamos con las notas numéricas
numericas <- Datos %>%
  select(ends_with(".num"))
# Calculamos matrices

covarianza <- cov(numericas, use = "complete.obs") #complete.obs no tiene en cuenta los NA
correlacion_pearson <- cor(numericas, use = "complete.obs", method = "pearson")

covarianza
correlacion_pearson

correlacion_spearman <- cor(numericas, use = "complete.obs", method = "spearman")
...
Dibuja las matrices de correlación del conjunto de notas numéricas. Podemos usar `pairs` del paquete base o bien `ggcorr` y `ggpairs` de la librería `GGally`.
...
chisq.test(tabla_alg_anm, correct = FALSE)
```
El valor p es mucho menor que 0.05, así que rechazamos la hipótesis nula de independencia. Las calificaciones en ALG y ANM no son independientes. Las notas de una asignatura están relacionadas con las de la otra; quien aprueba en ALG tiende a hacerlo también en ANM.

```{r Solucion_Ej9b, include=TRUE, echo=TRUE, eval=TRUE, results=TRUE}
tabla_alg_sex <- table(Datos$ALG.cat, as.factor(Datos$Sex))
chisq.test(tabla_alg_sex, correct = FALSE)
```
Las calificaciones en ALG y el sexo no muestran dependencia estadísticamente significativa.No hay evidencia de que el rendimiento en ALG dependa del sexo.
